...
class BicepCurlDetection:
    ...
    def handle_detected_results(self, video_name: str, motion_score: float = 0) -> tuple:
        """
        Save frame as evidence and apply fallback rep estimation
        """
        # STEP 1: Fallback Rep Estimation
        left_count = self.left_arm_analysis.get_counter()
        right_count = self.right_arm_analysis.get_counter()
        is_estimated = False

        if left_count == 0 and right_count == 0:
            # Fallback 1: Motion-based baseline
            if motion_score > 5.0:
                print(f"[DEBUG] Fallback: Enforcing 1 rep due to motion score {motion_score:.2f}")
                left_count = 1
                is_estimated = True

            # Fallback 2: Angle oscillation peaks (Check both arms)
            for side, history in [("left", self.left_angle_history), ("right", self.right_angle_history)]:
                if len(history) > 20:
                    peaks = 0
                    state = "down"
                    for angle in history:
                        if state == "down" and angle < 80:
                            state = "up"
                        elif state == "up" and angle > 110:
                            state = "down"
                            peaks += 1
                    
                    if side == "left" and peaks > left_count:
                        left_count = peaks
                        is_estimated = True
                    elif side == "right" and peaks > right_count:
                        right_count = peaks
                        is_estimated = True
            
            if is_estimated:
                print(f"[DEBUG] Fallback: Estimated L={left_count}, R={right_count} reps from elbow angle oscillations")
        ...
        metadata = {
            "counter": {
                "left_counter": left_count,
                "right_counter": right_count
            },
    ...
    def detect(
        ...
        try:
            ...
            # Track angles for fallback
            # Left arm
            left_shoulder = [landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x, landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].y]
            left_elbow = [landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].x, landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].y]
            left_wrist = [landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].x, landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].y]
            self.left_angle_history.append(calculate_angle(left_shoulder, left_elbow, left_wrist))

            # Right arm
            right_shoulder = [landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x, landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y]
            right_elbow = [landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].x, landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].y]
            right_wrist = [landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].x, landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].y]
            self.right_angle_history.append(calculate_angle(right_shoulder, right_elbow, right_wrist))
