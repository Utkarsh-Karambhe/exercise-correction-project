...
class SquatDetection:
    ...
    def handle_detected_results(self, video_name: str, motion_score: float = 0) -> tuple:
        """
        Save error frame as evidence and apply fallback rep estimation
        """
        ...
        if rep_count == 0:
            ...
            if motion_score > 5.0:  # Threshold for "definite movement"
                rep_count = 1
                is_estimated = True
                print(f"[DEBUG] Fallback: Enforcing 1 rep due to motion score {motion_score:.2f}")

            # Fallback 2: Angle oscillation peaks
            if len(self.knee_angle_history) > 20:
                # Count how many times the angle went below 140 (squat depth) and back up
                peaks = 0
                state = "up"
                for angle in self.knee_angle_history:
                    if state == "up" and angle < 140:
                        state = "down"
                    elif state == "down" and angle > 160:
                        state = "up"
                        peaks += 1
                ...
                if peaks > rep_count:
                    rep_count = peaks
                    is_estimated = True
                    print(f"[DEBUG] Fallback: Estimated {peaks} reps from knee angle oscillations")
